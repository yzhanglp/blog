...
- Binary source:
 ...
 - Can generlize to uniform distribution random varible:  
$H(X) = \log{(\left |M\right |)}$ with all event happened in probability $\frac{1}{\left |M\right |}$ (Also largest entropy with M events)
